...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
				cm.sendNext("我是金杯抽奖机.\r\n如果你有20个可以在我这里抽奖哦~!#r呵呵.运气好的话可以抽到GM帽子和极品装备喔~当然也有可能抽到金杯,祝你好运!#k\r\n金杯样子为这样的: #v4000038#");
			}
		else if (status == 1) {
			if (cm.haveItem(4000038,20)) {
			cm.sendYesNo("我看见你有20个金杯啦,你要试试运气么?");
			}
			else if (!cm.haveItem(4000038,20)) {
			cm.sendOk("你身上没有20个金杯#bGM#k请联系GM购买吧!!.");
			cm.dispose();
			}
		}
		else if (status == 2) {
